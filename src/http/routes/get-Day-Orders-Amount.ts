...
export const getDayOrderMonth = new Elysia()
  .use(auth)
  .get('/metrics/day-orders-month', async ({ getCurrentUser }) => {
    ...
    const today = dayjs() // todo: hoje
    const yesterday = today.subtract(1, 'day') // todo: ontem
    const startOfYesterday = yesterday.startOf('day') // todo: inicio de ontem
